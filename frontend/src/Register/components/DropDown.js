...
class Drop extends Component {
	state = {
		category: [
			...
			{ id: 4, cat: 'Catering' },
			{ id: 5, cat: 'Carpentry' },
			{ id: 6, cat: 'Electronics' },
			{ id: 7, cat: 'Fashion' },
			{ id: 8, cat: 'Catering' },
			{ id: 9, cat: 'Carpentry' },
			{ id: 10, cat: 'Electronics' },
			{ id: 11, cat: 'Fashion' },
			{ id: 12, cat: 'Catering' }
		],
		isFalse: false,
			...
			<div className="drop-wrapper">
				<div className="drop">
					...
					<an style={{ cursor: 'pointer' }}>
						<i onClick={this.handleDrop} className="fas fa-sort-down" />
					</an>
				</div>
				{this.state.isFalse ? (
					<div className="cat">
						{this.state.category.map((data) => (
							<div>
								<aside onClick={() => this.handleChangeBiz(data.cat)}>{data.cat}</aside>
							</div>
						))}
					</div>
				) : null}
